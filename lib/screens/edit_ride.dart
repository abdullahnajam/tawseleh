...
import '../provider/create_ride_provider.dart';
...
class _EditRideScreenState extends State<EditRideScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_){
      ...
      provider.reset();
      provider.setDeparture('${widget.ride.departureCity}:${widget.ride.departureNeighbour}');
      provider.setArrival('${widget.ride.arrivalCity}:${widget.ride.arrivalNeighbour}');
    });
    setState(() {
      url=widget.ride.image;
      selectedGender=widget.ride.gender.tr();
      gender=widget.ride.gender;
      startDate=DateTime.fromMillisecondsSinceEpoch(widget.ride.startDateInMilli);
      endDate=DateTime.fromMillisecondsSinceEpoch(widget.ride.endDateInMilli);
    });
    _dateController.text=widget.ride.date;
    _timeController.text=widget.ride.time;
    _departureController.text='${widget.ride.departureCity.tr()} ${'to'.tr()} ${widget.ride.departureNeighbour.tr()}';
    _arrivalController.text='${widget.ride.arrivalCity.tr()} ${'to'.tr()} ${widget.ride.arrivalNeighbour.tr()}';
    _priceController.text=widget.ride.pricePerSeat.toString();
    _seatsController.text=widget.ride.seats.toString();
  }
    ...
    return Scaffold(
      body: SafeArea(
        child: Column(
          ...
          children: [
            CustomAppBar(title: 'Edit the Ride'.tr()),
            Expanded(
              child: Form(
                key: _formKey,
                child: ListView(
                    ...
                    Center(
                      child: InkWell(
                            ...
                            if(url==''){
                              ...
                              await FirebaseFirestore.instance.collection('rides').doc(widget.ride.id).update({
                                ...
                                CoolAlert.show(
                                  ...
                                  text: 'Ride Successfully Updated'.tr(),
                                );
                        ...
                        child: Container(
                          ...
                          child: Text('Update'.tr(),style: TextStyle(fontSize:16,color: Colors.white),),
                        ),
                      ),
                    ),
                  ],
